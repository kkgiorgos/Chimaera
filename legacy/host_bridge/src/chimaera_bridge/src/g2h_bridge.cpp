...
class G2HBridge : public rclcpp::Node
{
public:
    G2HBridge()
    : Node("g2h_bridge")
...
private:
    ...
    void setup_socket()
    {
        ...
        strncpy(addr.sun_path, G2H_SOCKET_PATH, sizeof(addr.sun_path) - 1);
        ...
        RCLCPP_INFO(get_logger(), "Server listening on %s", G2H_SOCKET_PATH);
    }
...
int main(int argc, char ** argv)
{
    ...
    try {
        rclcpp::spin(std::make_shared<G2HBridge>());
    } catch (const std::exception & ex) {
        RCLCPP_ERROR(rclcpp::get_logger("g2h_bridge"), "Fatal error: %s", ex.what());
        rclcpp::shutdown();
        return EXIT_FAILURE;
    }
